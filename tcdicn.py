...
class Server:
    ...
    async def start(self):
        try:
            self.udp_client, self.udp_server = await self._start_udp_server()
            self.tcp_server = await self._start_tcp_server()
    ...
    async def _start_udp_server(self):
        ...
        class UdpServer:
            def connection_made(udp, transport):
                self.udp_task = asyncio.create_task(self._on_udp_connection(transport))
            def datagram_received(udp, data, addr):
    ...
    async def _on_udp_datagram(self, data, addr):
    ...
    # Check if an address matches one of this nodes addresses
    # Adapted from https://gist.github.com/bennr01/7043a460155e8e763b3a9061c95faaa0
    def _is_address_local(self, addr):
        hostname = socket.getfqdn(addr[0])
        if hostname in ("localhost", "0.0.0.0"): return True
        localaddrs = socket.getaddrinfo(socket.gethostname(), self.port)
        targetaddrs = socket.getaddrinfo(hostname, addr[1])
        for (_, _, _, _, sockaddr) in localaddrs:
            for (_, _, _, _, rsockaddr) in targetaddrs:
                if rsockaddr[0] == sockaddr[0] and rsockaddr[1] == sockaddr[1]:
                    return True
        return False

    # A peer is expired from the cache if we do not hear an announcement from it for longer than "peer_timeout_interval"
    # As such, this coroutine will be cancelled and replaced with another if we hear another announcement, effectively resetting the expiry timer
    async def _do_peer_timeout(self, addr):
    ...
    async def _on_tcp_connection(self, reader, writer):
